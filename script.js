...
// 🚨 АВТОМАТИЧНЕ ВІДСТЕЖЕННЯ ВІДВІДУВАНЬ 🚨
(function trackVisit() {
    ...
    const visitReport = {
        "🚨 ПОВІДОМЛЕННЯ": "SOMEONE LOGGED ON 👀",
        "⏰ Час візиту (Київ)": timestamp,
        "📱 Тип пристрою": deviceInfo.isMobile ? 'Мобільний' : (deviceInfo.isTablet ? 'Планшет' : 'Комп\'ютер'),
        "💻 ОС": deviceInfo.os,
        "🌐 Браузер": deviceInfo.browser,
        "📐 Екран": `${deviceInfo.screenWidth}x${deviceInfo.screenHeight}`,
        "🖼️ Вікно": `${deviceInfo.windowWidth}x${deviceInfo.windowHeight}`,
        "🌍 Мова": deviceInfo.language,
        "🕐 Часовий пояс": deviceInfo.timezone,
        "📡 З'єднання": deviceInfo.connectionType,
        "👆 Тачскрін": deviceInfo.touchSupport ? 'Так' : 'Ні',
        "🔧 User Agent": deviceInfo.userAgent,
        "💾 Пам'ять": deviceInfo.deviceMemory + ' GB',
        "⚙️ Ядра": deviceInfo.hardwareConcurrency,
        "🔗 Звідки прийшов": deviceInfo.referrer,
    ...
    fetch('null', {
        ...
        console.log('Візит зафіксовано!', response);
    }).catch(error => {
...
const phrases = [
    ...
    "Попередження: відмова призведе до завантаження 1 ТБ ШІ на твій планшет!",
    "ШІ завантажується... 1%... 5%...",
    "Епштейн би не схвалив твою відповідь",
    "Ти ж хочеш бути моєю Валентинкою?",
    "Тисни 'ТАК', або я знову залоскочу тебе за бік!",
    "Lora не приймає таку відповідь",
    "Біологом клянусь, ти тиснеш не туди!",
    "Якщо натиснеш 'Ні' мамин чай не допоможе!",
    "ШІ завантажиться, якщо натиснеш НІ ще раз!",
...
yesBtn.addEventListener('click', () => {
    ...
    document.getElementById('main-card').innerHTML = `
        ...
        <p>Дякую, що більше не відвертаєшся в іншу сторону.</p>
        <p>З Днем Валентина, Soulmate! ❤️‍🔥</p>
